...
// ===== 抽一句话 =====
els.drawBtn.addEventListener('click', drawQuote);

// ===== 喜欢 / 不爱 =====

function getQuoteStats() {
    return getData(STORAGE_KEYS.QUOTE_STATS, {});
}

function setQuoteStats(stats) {
    setData(STORAGE_KEYS.QUOTE_STATS, stats);
}
...
// 初始化统计（清零旧的模拟基础数据）
function initQuoteStats() {
    const version = getData(STORAGE_KEYS.QUOTE_STATS_VERSION, '');
    if (version !== 'zero-v1') {
        setQuoteStats({});
        setData(STORAGE_KEYS.QUOTE_STATS_VERSION, 'zero-v1');
        return {};
    }
    return getQuoteStats();
}

function updateStat(index, field, delta) {
    const stats = getQuoteStats();
    if (!stats[index]) stats[index] = { draws: 0, likes: 0, dislikes: 0 };
    stats[index][field] = Math.max(0, (stats[index][field] || 0) + delta);
    setQuoteStats(stats);
}
...
els.likeBtn.addEventListener('click', () => {
    ...
    if (prevChoice === 'like') {
        ...
        updateStat(lastQuoteId, 'likes', -1);
    } else {
        if (prevChoice === 'dislike') {
            updateStat(lastQuoteId, 'dislikes', -1);
        }
...
function drawQuote() {
    ...
    } while (quoteIdMap[idx] === currentId && quotes.length > 1);

    lastQuoteId = quoteIdMap[idx];
    currentQuote = quotes[idx];
    updateStat(lastQuoteId, 'draws', 1);
...
async function finishCheckin(type) {
    ...
    const todayIdx = checkins.findIndex(c => c.date === dateStr);
    ...
    if (todayIdx >= 0) {
        // 今天已经打过卡，更新
        checkins[todayIdx] = { ...checkins[todayIdx], ...checkinData };
    } else {
    ...
    if ((type === 'voice' || type === 'both') && currentRecordingBlob) {
        ...
        // 更新打卡记录中的 recordingId
        const updatedCheckins = getData(STORAGE_KEYS.CHECKINS, []);
        const idx = updatedCheckins.findIndex(c => c.date === dateStr);
        if (idx >= 0) updatedCheckins[idx].recordingId = recordingId;
...
function getCheckinCounts() {
    ...
    const counts = {};
    checkins.forEach(c => {
        const idx = quotes.indexOf(c.quote);
        if (idx >= 0) {
...
function renderAdminQuotes() {
    ...
    const rows = quotes.map((quote, index) => {
        ...
        const stat = stats[id] || { draws: 0, likes: 0, dislikes: 0 };
        const checkins = checkinCounts[index] || 0;
        return { quote, index, stat, checkins };
    });

    if (adminSortBy !== 'default') {
        rows.sort((a, b) => {
            let aVal, bVal;
            if (adminSortBy === 'checkins') {
                aVal = a.checkins; bVal = b.checkins;
            } else {
                aVal = a.stat[adminSortBy] || 0; bVal = b.stat[adminSortBy] || 0;
            }
    ...
    rows.forEach(({ quote, index, stat, checkins }) => {
        const div = document.createElement('div');
        ...
        div.innerHTML = `
            ...
            <div class="admin-stat-num">${checkins}</div>
