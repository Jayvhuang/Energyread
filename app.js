...
async function cleanupCloudData() {
    ...
    try {
        // 查出要删除的录音记录，先删 Storage 文件
        const { data: oldVoice } = await sb
            .from('garden_items')
            .select('audio_url')
            .lt('created_at', cutoff)
            .not('audio_url', 'is', null);
        if (oldVoice && oldVoice.length > 0) {
            const paths = oldVoice
                .map(r => r.audio_url?.match(/\/audio\/(.+)/)?.[1])
                .filter(Boolean);
            if (paths.length > 0) {
        ...
        await sb.from('garden_items').delete().lt('created_at', cutoff);
    } catch (err) {
