...
function renderAdminQuotes() {
    ...
    rows.forEach(({ quote, index, stat }) => {
        ...
        if (entry.status === 'banned') {
            badgeHtml = ' <span class="admin-source-badge banned">冷宫</span>';
        } else if (entry.source === 'user') {
            badgeHtml = ' <span class="admin-source-badge user">投稿</span>';
        } else if (entry.source === 'promoted') {
            badgeHtml = ' <span class="admin-source-badge promoted">已转正</span>';
        }

        div.innerHTML = `
            <div class="admin-quote-text">${escapeHtml(quote)}${badgeHtml}</div>
